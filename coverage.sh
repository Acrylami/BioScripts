...
for f in $input
do
 printf "$f" >> "${inpath}/input_bams.list"
 printf "\n" >> "${inpath}/input_bams.list"
...
gatk --java-options "-Xmx4g" DepthofCoverage -R "${inpath}/NC_045512_DNA.fasta" -I "${inpath}/input_bams.list" -O "${inpath}/covid_coverage" --output-format csv
